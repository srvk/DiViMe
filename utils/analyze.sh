...
ls /vagrant/${DATA_FOLDER}/detailed_outputs/yunitator*.rttm | sed "s/.*english_//g" > files.txt

cat /vagrant/${DATA_FOLDER}/files.txt | while read -r line ; do
	#get first five cols & speaker col
	awk '{print $1 "\t" $2 "\t" $3 "\t" $4 "\t" $5 "\t" $8}' < /vagrant/${DATA_FOLDER}/detailed_outputs/yunitator_english_$line > base.temp
	...
	nchi=`wc -l vcm.temp | awk '{print $1}'`
	nall=`wc -l base3.temp | awk '{print $1}'`
	...
	paste /vagrant/${DATA_FOLDER}/base3.temp /vagrant/${DATA_FOLDER}/vcm.temp > b/vagrant/${DATA_FOLDER}/ase4.temp
